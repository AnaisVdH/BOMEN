...
void setup() {
  ...
  size(1009, 678, P3D);
  smooth(8);
  println("Hei Andrew en Jan! geniet ervan");
...
void makeTrees(float amount) {
  ...
  for (int i = 0; i<amount; i++) {
    ...
    boomz = int(random(50, 100));

    bomen.add(new Boom(boomx, boomy, boomz)); //bomen worden aangemaakt in klasse als repellers
  }
